...
selected_samples_FvsSI <- c(
  "C56", "C6", "C100", "C84", "C39", "C66", "C45", "C8", "C41", "C54",
  "C65", "C2", "C44", "C31", "C105", "C19", "C32", "C68", "C97", "C72",
  "C13", "C42", "C20", "C49", "C4", "C52", "C79", "C90", "C67", "C5",
  "A1", "A3", "A4", "A7", "A9", "A11", "A12", "A14", "A15", "A16", "A17",
  "A18", "A20", "A21", "A23", "A25", "A26", "A28", "A31", "A32", "A33",
  "A34", "A38", "A39", "A40", "A41", "A42", "A43", "A44", "A45", "A46",
...
#######################################
### FERTILITY vs SIMPLE INFERTILITY ###
#######################################

v <- v[, selected_samples_FvsSI, drop=FALSE]

#reading pheno
targets_FvsSI<-read.table(file = "targets_ferti_simple.txt",header = T,stringsAsFactors=F)
targets_FvsSI
colnames(rawdata)
targets_FvsSI$Filename
Group<-factor(targets_FvsSI$Group)
...
classes.DIAGNOSIS <- targets_FvsSI$Group
names(classes.DIAGNOSIS) <- targets_FvsSI$Filename
...
library(writexl)
write_xlsx(featTable, "FERTILEvsSIMPLE_INFERTILITY_featTable.xlsx")

decoReport(deco.results.ma, sub_binary , pdf.file = "Report_FERTILITYvsSIMPLE_INFERTILITY.pdf", cex.names = 0.3, print.annot=T)