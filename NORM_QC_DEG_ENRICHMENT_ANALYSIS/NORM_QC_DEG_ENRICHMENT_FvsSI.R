...
rawdata <- subset(rawdata, select = -A2)
rawdata <- subset(rawdata, select = -A8)
rawdata <- subset(rawdata, select = -A13)
rawdata <- subset(rawdata, select = -A19)
rawdata <- subset(rawdata, select = -A27)
rawdata <- subset(rawdata, select = -A29)
rawdata <- subset(rawdata, select = -A30)
rawdata <- subset(rawdata, select = -A35)
rawdata<-rawdata[,sort(colnames(rawdata))]


#reading pheno
targets <- read.table(file = "targets_comp.txt", header = T, stringsAsFactors = F)
filenames_to_remove <- c("A2", "A8", "A13", "A19", "A27", "A29", "A30", "A35")
...
contrast<-makeContrasts(
  b=(GroupSIMPLE_INFERTILITY) - (GroupFERTILE),
  levels=design
)
...
sample<-mean(as.numeric(rpkm["RN7SL2",targets[targets$Group=="SIMPLE_INFERTILITY","Group"]]), na.rm = TRUE)
#logFC
log2(sample/control)


write.table(top1.table,file="FERTILE_vs_SIMPLE_INFERTILITY.xls",quote = F,sep="\t",col.names = NA)

VolcanoFig(top1.table, "FERTILE_vs_SIMPLE_INFERTILITY.pdf", "FERTILE_vs_SIMPLE_INFERTILITY")
...
my_enrichment(top1.table, P.Value=0.05, FA_label="enrichment_FERTILE_vs_SIMPLE_INFERTILITY", cutoff=1,organism="human",FC=1.5)
