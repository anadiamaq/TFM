...
deco_data <- readxl::read_excel("FERTILEvsSIMPLE_INFERTILITY_featTable.xlsx") %>%
...
my_enrichment <- function(de_data, Profile = "Majority", P.Value = 0.05, FA_label, cutoff = 0.05, organism = "human", FC = 1.5) {
  
  # Filter the data for the profile "Majority"
  filtered_data <- de_data %>% filter(Profile == "Majority")
...
my_enrichment(deco_data, P.Value=0.05, Profile = "Majority", FA_label="enrichment_FERTILE_vs_SIMPLE_INFERTILITY_DECO", cutoff=1,organism="human",FC=1.5)
