...
lauVolcano<-function(data,file,main){
  ...
  g = ggplot(data=data, aes(x=logFC, y=-log10(P.Value), colour=threshold)) + coord_cartesian(xlim = c(-10, 10)) +
    ...
    geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "black", size = 0.5) +  # P.Value umbral (0.05)
    geom_vline(xintercept = 0.58, linetype = "dashed", color = "black", size = 0.5) +  # logFC positivo umbral
    geom_vline(xintercept = -0.58, linetype = "dashed", color = "black", size = 0.5) +  # logFC negativo umbral
...
rawdata <- subset(rawdata, select = -C66)
rawdata <- subset(rawdata, select = -C45)
rawdata <- subset(rawdata, select = -C8)
rawdata <- subset(rawdata, select = -C4)
rawdata <- subset(rawdata, select = -A1)
rawdata <- subset(rawdata, select = -A7)
rawdata <- subset(rawdata, select = -A16)
rawdata <- subset(rawdata, select = -A45)
rawdata <- subset(rawdata, select = -A46)
rawdata <- subset(rawdata, select = -A48)
rawdata<-rawdata[,sort(colnames(rawdata))]


#reading pheno
targets <- read.table(file = "targets_comp.txt", header = T, stringsAsFactors = F)
filenames_to_remove <- c("C66", "C45", "C8", "C4", "A1", "A7", "A16", "A45", "A46", "A48")
...
keep<-filterByExpr(x,design) #VAMOS A DEJAR EL FILTERBYEXPR, SI SE VAN TODOS LOS GENES ME DICES
...
# v <- voom(counts, design, plot=TRUE, normalize="quantile") if data is very noisy
...
resultsfile<-file.path(paste(label,"_RPKM_FvsCI.xls", sep=""))
write.table(rpkm, file=resultsfile, sep = "\t", col.names = NA , qmethod = "double") 
resultsfile

fit <- lmFit(v, design) #
contrast<-makeContrasts(
  b=(GroupCOMBINED_INFERTILITY) - (GroupFERTILE),
  levels=design
)
...
control<-mean(as.numeric(rpkm["RN7SL2",targets[targets$Group=="FERTILE","Group"]]), na.rm = TRUE)
sample<-mean(as.numeric(rpkm["RN7SL2",targets[targets$Group=="COMBINED_INFERTILITY","Group"]]), na.rm = TRUE)
#logFC
log2(sample/control)


write.table(top1.table,file="FERTILE_vs_COMBINED_INFERTILITY.xls",quote = F,sep="\t",col.names = NA)

lauVolcano(top1.table, "FERTILE_vs_COMBINED_INFERTILITY.pdf", "FERTILE_vs_COMBINED_INFERTILITY")
...
my_enrichment(top1.table, P.Value=0.05, FA_label="enrichment_FERTILE_vs_COMBINED_INFERTILITY", cutoff=1,organism="human",FC=1.5)
