...
deco_data <- readxl::read_excel("SIMPLE_INFERTILITYvsCOMBINED_INFERTILITY_featTable.xlsx") %>%
  select(SYMBOL, Profile, Avrg.logFC, Best.adj.P.Val) %>%
  rename(Gene = SYMBOL, logFC = Avrg.logFC, P.Value = Best.adj.P.Val)
...
my_enrichment <- function(de_data, Profile = c("Majority", "Complete"), P.Value = 0.05, FA_label, cutoff = 0.05, organism = "human", FC = 1.5) {
  
  # Filter the data for the profile "Majority"
  filtered_data <- de_data %>% filter(Profile %in% c("Majority", "Complete"))
  
  gene <- filtered_data[filtered_data$P.Value <= P.Value & abs(filtered_data$logFC) >= 1.5, "Gene"]
  gene <- gene$Gene
...
my_enrichment(deco_data, P.Value=0.05, Profile = c("Majority", "Complete"), FA_label="enrichment_SIMPLE_INFERTILITY_vs_COMBINED_INFERTILITY_DECO", cutoff=1,organism="human",FC=1.5)
